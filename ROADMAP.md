...
### Secção 12c — Modelos em inglês no "Enviar alerta"
...
em `notifications`).
**Objetivo:** O modal "Enviar alerta" pré-preenche o título e a mensagem em
inglês para os clientes com `locale: 'en'` — "Free checkup for your
car/floor", "Free wash for your …", "Message from Marble Studios", no tom
de `functions/src/texts.ts` — com um campo "Idioma do texto modelo" (PT/EN)
...
editar. Os textos PT ficam como estavam. Ver o README do backoffice
("Acompanhamento pós-serviço e push" e `clients`).
**Decisões (2026-09-07, Fábio):** pré-preencher em EN em vez de só avisar;
seletor além do automático; títulos EN a variar com o carro/chão como nas
Functions; ROADMAP + DEVELOPMENT.md + comentário em `texts.ts` atualizados.
