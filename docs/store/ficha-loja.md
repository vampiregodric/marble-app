...
Perfil e os Alertas. **Criada a 2026-09-08 no prod** com `npm run demo:account`
(`scripts/demo-account.mjs`): `revisao@marble.pt`, consentimento na versão
legal atual, um carro com checkup pendente, um chão em dia e dois alertas.
**A password está no gestor de passwords do Fábio** — nunca neste ficheiro
nem em mais nada que vá para o git (esteve aqui até 2026-09-13; a auditoria
de 2026-09-12, SEG-C-01, apanhou-a num repositório que estava público, e
foi trocada nesse dia). Ao colar os textos abaixo nas consolas, substitui
"(password)" pela password real; nas consolas ela fica em campos privados
dos revisores. Se a password mudar, corre o script outra vez com `--apply`,
guarda a nova no gestor e atualiza as duas consolas. Texto:

```
Início, Portfólio, Serviços e Eventos não pedem conta. Perfil e Alertas pedem
login (email/password). Conta de teste: revisao@marble.pt / (password). A conta tem um
...
Alerts require login (email/password). Test account: revisao@marble.pt / (password).
